...
unsigned char __attribute__((section(CONFIG_ET_METHOD_SECTION),
...
Result<BufferCleanup> prepare_input_tensors(
  ...
  for (size_t i = 0; i < num_inputs; i++) {
    ...
    if (err != Error::Ok) {
      ET_LOG(
          Error, "Failed to prepare input %zu: 0x%" PRIu32, i, (uint32_t)err);
...
int et_runner(float v) {
  ...
  for (size_t id = 0; id < num_memory_planned_buffers; ++id) {
    ...
    if(buffer == nullptr) {
        ET_LOG(Error, "Could not allocate memory for memory planned buffer size %zu", buffer_size);
        return -1;
    }
        ...
        for (int j = 0; j < tensor.numel(); ++j) {
          if (tensor.scalar_type() == ScalarType::Int) {
            ...
            printf(
                ...
                i,
                j,
                (double)tensor.const_data_ptr<float>()[j]);
