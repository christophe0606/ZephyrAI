...
void interrupt_thread_function(void *, void *, void *)
{
    ...
    for (;;) {
        ...
        arm_cmsis_stream::EventQueue::cg_eventQueue->push(LocalDestination{videoNode, 0},
                                                          std::move(evt));
    }
#endif
}
...
void stream_thread_function(void *, void *, void *)
{
    ...
    // We don't free the scheduler nodes
    // since in a pure event based graph the nodes
    // must continue to exist to process events.
}
...
int init_stream()
{
    ...
    event_pool_buffer =
            k_heap_alloc(&sram0_heap, NB_MAX_EVENTS * (sizeof(ListValue) + 16), K_NO_WAIT);
    ...
    buf_pool_buffer =
            k_heap_alloc(&sram0_heap, NB_MAX_BUFS * (sizeof(Tensor<double>) + 16), K_NO_WAIT);
    ...
    tid_interrupts = k_thread_create(&interrupt_thread, interrupt_thread_stack,
                                     ...
                                     interrupt_thread_function, NULL, NULL, NULL, NORMAL_PRIORITY,
                                     0, K_NO_WAIT);

    k_thread_name_set(&interrupt_thread, "interrupt_to_evt");

    cg_eventThread = k_thread_create(
            &event_thread, event_thread_stack, K_THREAD_STACK_SIZEOF(event_thread_stack),
            event_thread_function, NULL, NULL, NULL, NORMAL_PRIORITY, K_FP_REGS, K_NO_WAIT);
    ...
    tid_stream = k_thread_create(&stream_thread, stream_thread_stack,
                                 ...
                                 NULL, NULL, NULL, AUDIO_THREAD_PRIORITY, K_FP_REGS, K_NO_WAIT);

    k_thread_name_set(&stream_thread, "stream_thread");

    LOG_INF("Stream initialized\n");
