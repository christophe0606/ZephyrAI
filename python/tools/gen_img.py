...
parser = argparse.ArgumentParser(description='Generate images for the KWS demo')
parser.add_argument('-b', action='store_true', help="Generate binary files")
...
def gen_c_file(write_bin=True):
    c_out_file = f"src/streamgraph/nodes_src/kws_img.c"
    h_out_file = f"src/streamgraph/streamnodes/appnodes/kws_img.h"
...
def gen_bin_file():
    for word in words:
        img_array = generate_image(word,disp=False,font_size=70)
        img_array.tofile(f"assets/{word.lower()}.bin")
       
if args.b:
    gen_bin_file()    
    # Only dimension of pictures is written to the files
    gen_c_file(write_bin=False)
else:
    gen_c_file()
