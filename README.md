...
```shell
docker build -f executorch/zephyr/.docker/DockerfileSimple --progress=plain -t executorch-arm-builder .
```

run
```shell
docker run --rm --user 1001:1001 -v %cd%/executorch:/workspace2 executorch-arm-builder:latest bash /workspace2/zephyr/scripts/local_workflow.sh
```

```shell
docker run --rm --user 1001:1001 -v %cd%/executorch:/workspace2 executorch-arm-builder:latest bash /workspace2/zephyr/scripts/build.sh
```

```shell
docker run --rm --user 1001:1001 -it -v %cd%/executorch:/workspace2 executorch-arm-builder:latest 
```