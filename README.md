...
The Python script `src/streamgraph/python/camera_test.py` is a work in progress for testing camera support. It is mapped to application `appc`.
...
If you want to enable the code size optimization, use the scripts (`kws.py`, `spectrogram.py`, `camera_test.py` ...) with `--size` options.

And when you have generated all graphs, use:

```shell
python ./src/streamgraph/python/generate.py appa appb appc
```
